...
const config = {
  ...
  try {
    ...
    const promises = sampleData.map(event => axios.post(`${process.env.GRAPH_API_HOST}/v1.0/me/calendar/events`, JSON.parse(JSON.stringify(event)), config));
    const values = await Promise.all(promises)
    console.log(process.env.GRAPH_API_HOST)
